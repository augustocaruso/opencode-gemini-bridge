...
export function runDoctor(options: DoctorOptions = {}): DoctorReport {
  ...
  recoverStaleStartupStatus({
    ...
    lockPath: path.join(paths.generatedDir, "ogb-startup-sync.lock"),
  ...
  const homeGlobalConfigOk = paths.homeMode
    ...
    && configReferencesInstruction(opencodeConfig, paths.expandedGeminiPath);
  ...
  const startupSync = {
    projectPlugin: !paths.homeMode && fs.existsSync(path.join(paths.projectRoot, ".opencode", "plugins", "ogb-startup-sync.js")),
    projectConfig: !paths.homeMode && fs.existsSync(path.join(paths.projectRoot, ".opencode", "generated", "ogb-startup-sync.json")),
    globalPlugin: fs.existsSync(path.join(globalOpenCodeConfigDir({ homeDir: paths.homeDir }), "plugins", "ogb-startup-sync.js")),
    globalConfig: fs.existsSync(path.join(paths.generatedDir, "ogb-startup-sync.json")),
  ...
  const globalStartupPluginPath = path.join(globalOpenCodeConfigDir({ homeDir: paths.homeDir }), "plugins", "ogb-startup-sync.js");
  const globalStartupPluginConfigured = configHasPluginSpec(
    globalOpenCodeConfigPath(paths.homeDir),
    pathToFileURL(globalStartupPluginPath).href,
  );
  ...
  if (paths.homeMode) {
    if (hasGlobalGeminiContextSource(paths.homeDir, inv)) {
      if (!expandedExists) warnings.push("Missing global expanded Gemini context. Run ogb sync.");
      else if (!expandedGeminiHasMarker) warnings.push("Global expanded GEMINI file is missing generated DO NOT EDIT marker. Run ogb sync.");
      else if (expandedGeminiVersion && expandedGeminiVersion !== OGB_VERSION) warnings.push(`Global expanded GEMINI file was generated by ogb ${expandedGeminiVersion}; current ogb is ${OGB_VERSION}. Run ogb sync.`);

      if (!fs.existsSync(opencodeConfig)) warnings.push("Missing global OpenCode config. Run ogb setup-ux or ogb sync.");
      else if (!configReferencesInstruction(opencodeConfig, paths.expandedGeminiPath)) warnings.push("Global OpenCode config does not reference the OGB expanded Gemini context. Run ogb sync.");
    }
  ...
  if (!paths.homeMode && inv.extensions.length > 0 && !extensionCompatibility.mapExists) warnings.push("Missing .opencode/generated/ogb-extension-map.json. Run ogb sync.");
  ...
  if (!paths.homeMode && !rulesyncCommand) warnings.push("Rulesync is unavailable; ogb sync will use bridge-native projection only.");
