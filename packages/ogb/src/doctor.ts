...
export function runDoctor(options: DoctorOptions = {}): DoctorReport {
  ...
  if (paths.homeMode) {
    if (hasGlobalGeminiContextSource(paths.homeDir, inv)) {
      ...
      else if (!configReferencesInstruction(opencodeConfig, paths.expandedGeminiPath, paths.homeDir)) warnings.push("Global OpenCode config does not reference the OGB expanded Gemini context. Run ogb sync.");
    }
