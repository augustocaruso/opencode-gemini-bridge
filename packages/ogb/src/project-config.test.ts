...
test("ensureProjectConfig creates a conservative OpenCode config and avoids manual overwrite", () => {
  ...
  assert.equal(parsed.agent.agent.permission.bash["stat *"], "allow");
  assert.equal(parsed.agent.agent.permission.read, "allow");
  assert.equal(parsed.agent.agent.permission.glob, "allow");
  assert.equal(parsed.agent.agent.permission.grep, "allow");
  assert.equal(parsed.agent.agent.permission.list, "allow");
  ...
  assert.equal(parsed.agent.plan.permission.bash["stat *"], "allow");
  assert.equal(parsed.agent.plan.permission.bash["du *"], "allow");
  assert.equal(parsed.agent.plan.permission.bash["git cat-file*"], "allow");
  assert.equal(parsed.agent.plan.permission.bash["gh auth status*"], "allow");
  assert.equal(parsed.agent.plan.permission.bash["gh pr view*"], "allow");
  assert.equal(parsed.agent.plan.permission.bash["gh release view*"], "allow");
  assert.equal(parsed.agent.plan.permission.bash["gh run view*"], "allow");
  assert.equal(parsed.agent.plan.permission.bash["gh api repos/*"], "allow");
  assert.equal(parsed.agent.plan.permission.bash["gh pr checkout*"], "deny");
  assert.equal(parsed.agent.plan.permission.bash["gh release delete*"], "deny");
  assert.equal(parsed.agent.plan.permission.bash["gh api *--method POST*"], "deny");
  assert.equal(parsed.agent.plan.permission.read, "allow");
  assert.equal(parsed.agent.plan.permission.glob, "allow");
  assert.equal(parsed.agent.plan.permission.grep, "allow");
  assert.equal(parsed.agent.plan.permission.list, "allow");
