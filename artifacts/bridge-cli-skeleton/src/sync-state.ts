...
export function readSyncState(projectRoot = process.cwd()): SyncState | undefined {
  const statePath = resolveProjectPaths(projectRoot).syncStatePath;
...
export function writeSyncState(state: SyncState, projectRoot = process.cwd()): void {
  const statePath = resolveProjectPaths(projectRoot).syncStatePath;
