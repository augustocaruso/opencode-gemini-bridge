...
function readLimits(root) {
  const limits = safeReadJson(path.join(root, ".opencode", "generated", "ogb-limits.json"));
...
function readUiPrefs(root) {
  const prefs = safeReadJson(path.join(root, ".opencode", "generated", "ogb-ui.json"));
...
function readPanel(root) {
  const generated = path.join(root, ".opencode", "generated");
...
function ensureTuiConfig(options: { projectRoot: string; dryRun?: boolean; extraPlugins?: string[] }): TuiSidebarResult["config"] {
  const absPath = path.join(options.projectRoot, ...TUI_CONFIG_PATH.split("/"));
  const exists = fs.existsSync(absPath);
  ...
  if (next.error) {
    return {
      ...
      status: "conflict",
      message: `.opencode/tui.jsonc could not be updated: ${next.error}`,
    };
  }
  ...
  if (options.dryRun) {
    return {
      ...
      message: next.changed ? `Would ${exists ? "update" : "create"} ${TUI_CONFIG_PATH}` : `${TUI_CONFIG_PATH} already references ${TUI_SIDEBAR_PLUGIN_SPEC}`,
    };
  }
  ...
  if (!next.text) {
    return {
      ...
      relPath: TUI_CONFIG_PATH,
      status: "unchanged",
      message: `${TUI_CONFIG_PATH} already references ${TUI_SIDEBAR_PLUGIN_SPEC}`,
    };
  }

  if (!next.changed) {
    const state = readSyncState(options.projectRoot) ?? emptySyncState(OGB_VERSION);
    upsertManagedFile(state, { path: TUI_CONFIG_PATH, sha256: sha256Text(next.text), source: "ogb" });
    writeSyncState(state, options.projectRoot);
    return {
      path: absPath,
      relPath: TUI_CONFIG_PATH,
  ...
  return {
    ...
    relPath: TUI_CONFIG_PATH,
    status: exists ? "updated" : "created",
    message: `${exists ? "Updated" : "Created"} ${TUI_CONFIG_PATH}`,
  };
}
